...
class BitNetAttention(nn.Module):
    ...
    def __init__(self, model_args: TransformerModelArgs):
        ...
        self.wq_norm = build_norm(model_args.norm_type, dim=model_args.dim, eps=model_args.norm_eps)
        self.wk_norm = build_norm(model_args.norm_type, dim=model_args.dim, eps=model_args.norm_eps)
        self.wv_norm = build_norm(model_args.norm_type, dim=model_args.dim, eps=model_args.norm_eps)
    ...
    def forward(
        ...
        """
        bs, seqlen, _ = x.shape
        xq, xk, xv = self.wq(self.wq_norm(x)), self.wk(self.wk_norm(x)), self.wv(self.wv_norm(x))
...
class BitNetFeedForward(nn.Module):
    ...
    def __init__(
        ...
        self.w1_norm = build_norm(norm_type, dim=dim, eps=norm_eps)
        self.w2_norm = build_norm(norm_type, dim=hidden_dim, eps=norm_eps)
        self.w3_norm = build_norm(norm_type, dim=dim, eps=norm_eps)

    def forward(self, x):
        return self.w2(self.w2_norm(F.silu(self.w1(self.w1_norm(x))) * self.w3(self.w3_norm(x))))
